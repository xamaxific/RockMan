...
namespace Puppet2D
{
	public class Puppet2D_BakeAnimation : MonoBehaviour
	{
		...
		List<Transform> GetListBones()
		{
			...
			{
				if (child.transform.GetComponent<SpriteRenderer>() && child.transform.GetComponent<SpriteRenderer>().sprite && child.transform.GetComponent<SpriteRenderer>().sprite.name.Contains("ffd"))
				{
					returnList.Add(child.transform);				
				}
				else if (child.transform.GetComponent<Puppet2D_Bone>())
				{
					returnList.Add(child.transform);
				}
			}
